		...
		this.each(function() {
			...
			$.getJSON(lastUrl, function(data){ 
				$.each(data.recenttracks.track, function(i, item){
					...
					$current.find('[class=lfm_song]').append(song + "<img class='eq' src='/img/about/listening/icon_eq.gif' alt='now playing'>");
					$current.find('[class=lfm_artist]').append(artist);
					$current.find('[class=lfm_album]').append(album);
					$current.find('[class=lfm_art] a').append("<img src='"+art+"' alt='Artwork for "+album+"'/>");
